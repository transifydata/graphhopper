        ...
        while (!finished()) {
            ...
            while (iter.next()) {
                ...
                if (nextLabel == null) {
                    ...
                    if (getExploreValue(nextLabel) <= limit) {
                        nextLabel.consumed_part = iter.getDistance();
                    } else if (includeOverextendedEdges) {
                        // Overextended case: only part of the edge fits within the limit
                        double consumed = limit - getExploreValue(currentLabel);
                        if (consumed > 0) {
                            nextLabel.consumed_part = consumed;
                            consumer.accept(nextLabel);
                        }
                    }
